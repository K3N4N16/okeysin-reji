...
for i, msg in enumerate(st.session_state.history):
    if msg["role"] == "user":
        ...
        if msg.get("audio"):
            st.audio(msg["audio"], format="audio/wav", autoplay=(i == len(st.session_state.history)-1 and st.session_state.auto_play))
            c1, c2 = st.columns([2, 2])
            with c1:
                st.download_button("📥 Ses İndir", msg["audio"], f"dilay_{i}.wav", mime="audio/wav", key=f"dl_{i}")
            with c2:
...
with st.sidebar:
    ...
    if st.button("🗑️ Tüm Sohbeti Temizle"):
        ...
        st.rerun()
    st.divider()
    st.info("💡 Dilay artık senin klonladığın özel sesle konuşuyor!")
