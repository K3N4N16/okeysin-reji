...
st.markdown("<p style='text-align:center;color:#888;'>Kendi listelerin + Yapay Zeka ile sınırsız yayın</p>", unsafe_allow_html=True)
        ...
        with st.spinner("🌌 Radar taranıyor..."):
            try:
                ...
                for idx, item in enumerate(data.get("results", [])):
                    ...
                    with col2:
                        if st.button("▶️ Oynat", key=f"play_{idx}"):
...
if 'play_url' in st.session_state:
    ...
    st.info(f"Mod: **{mode.upper()}** | URL: {url[:80]}...")
    ...
    if mode == "youtube":
        ...
        if "watch?v=" in url:
            ...
            vid = url.split("/")[-1]
        st.components.v1.iframe(f"https://www.youtube.com/embed/{vid}?autoplay=1&rel=0", height=520)

    elif mode == "iframe":
        st.components.v1.iframe(url, height=520, scrolling=True)

    else:
        # HLS veya direkt video için güçlü player
        player_html = f"""
        <video id="myvideo" controls autoplay style="width:100%; background:#000; max-height:520px;">
        ...
        """
        st.components.v1.html(player_html, height=560)

    # Kontrol butonları
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔄 Başka Kaynak Dene"):
            for key in ['play_url', 'play_mode', 'play_title']:
